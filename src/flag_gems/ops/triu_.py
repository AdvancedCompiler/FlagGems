...
def triu_(A, diagonal=0):
    assert A.dim() >= 2, "Invalid dim: input must have at least 2 dimensions"
    ...
    if A.dtype in [torch.float16, torch.bfloat16]:
        dtype_convert = True
        tmp_A = A.to(torch.float32)
        compute_A = tmp_A.contiguous()
    else:
    ...
    with torch_device_fn.device(A.device):
        if compute_A.dim() == 2:
            grid = lambda meta: (triton.cdiv(M, meta["M_BLOCK_SIZE"]),)
            triu_kernel[grid](compute_A, compute_A, M, N, diagonal)
        else:
            batch = int(torch.numel(compute_A) / M / N)
            B = compute_A.view(batch, -1)
            grid = lambda meta: (
                triton.cdiv(batch, meta["BATCH_BLOCK_SIZE"]),
                triton.cdiv(M * N, meta["MN_BLOCK_SIZE"]),
            )
    ...
    if dtype_convert:
        origin.copy_(compute_A.to(origin.dtype))

        if tmp_A is not None:
            del tmp_A, compute_A

    logger.debug("GEMS TRIU_INPLACE")
    return origin
