...
class NLLLossNDFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, target, weight=None, reduction=1, ignore_index=-100):
        out, total_weight = nll_loss_nd_forward(
            input, target, weight, reduction, ignore_index
        )

        ctx.save_for_backward(input, target, weight, total_weight)
    ...
    def backward(ctx, grad_output):
        input, target, weight, total_weight = ctx.saved_tensors
        reduction = ctx.reduction
        ignore_index = ctx.ignore_index

        grad_input = nll_loss_nd_backward(
            grad_output,
            input,
            target,
            weight=weight,
...
def nll_loss_nd(
    ...
    return NLLLossNDFunction.apply(input, target, weight, reduction, ignore_index)
