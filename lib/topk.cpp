...
std::tuple<at::Tensor, at::Tensor> topk(const at::Tensor &x, int64_t k, int64_t dim, bool largest, bool sorted){
    ...
    at::Tensor stage1_out_idx = at::empty(batch_size * chunk_num * k, at::TensorOptions().dtype(torch::kLong).device(x.device()));
    ...
    at::Tensor stage2_out_idx = at::empty(out_shape, at::TensorOptions().dtype(torch::kLong).device(x.device()));
    const TritonJITFunction &f1 =
      TritonJITFunction::getInstance(std::string(utils::get_triton_src_path() / "topk.py"), "topk_stage1_kernel");
    const TritonJITFunction &f2 =
      TritonJITFunction::getInstance(std::string(utils::get_triton_src_path() / "topk.py"), "topk_stage2_kernel");
